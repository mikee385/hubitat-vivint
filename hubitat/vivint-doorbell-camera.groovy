...
String getVersionLabel() { return "Vivint Doorbell Camera, version ${getVersionNum()} on ${getPlatform()}" }

String getType() { return "DoorbellCamera" }
String[] getMotionValues() { return ["inactive", "active"] }

metadata {
    definition (
		name: "Vivint Doorbell Camera", 
		namespace: "mikee385", 
		author: "Michael Pierce", 
		importUrl: "https://raw.githubusercontent.com/mikee385/hubitat-vivint/master/hubitat/vivint-doorbell-camera.groovy"
	) {
	    capability "Actuator"
...
def update(deviceData) {
    ...
    if (deviceData.pushed != null) {
        if (deviceData.pushed == true) {
            sendEvent(name: "pushed", value: 1)
        } else if (deviceData.pushed != false) {
            log.error "Unknown value for pushed: ${deviceData.pushed}"
        }
    } else {
        log.warn "pushed not found in update."
    }
}

def push(buttonNumber) {
    log.warn "push command not implemented yet."
}